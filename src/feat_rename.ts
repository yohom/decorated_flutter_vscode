...
export async function handleFeatRename() {
  ...
  if (fs.existsSync(blocFilePath)) {
    fs.renameSync(
      blocFilePath,
      `${modulePath}/lib/src/bloc/local/${newName}.bloc.dart`
    );
  }
  ...
  fs.renameSync(
    screenFilePath,
    `${modulePath}/lib/src/ui/screen/${newName}/${newName}.screen.dart`
  );
  ...
  _replaceForFile(
    `${modulePath}/lib/src/bloc/bloc.export.dart`,
    oldName,
    newName
  );
  _replaceForFile(
    `${modulePath}/lib/src/ui/screen/screen.export.dart`,
    oldName,
    newName
  );
